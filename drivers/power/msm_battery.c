 ...
 *       - YangGang <yanggang@tcl.com> cancle delay
 *
 */
...
#define	MSM_BATT_POLLING_TIME		(20 * HZ)
...
#define BATT_LED_GPIO 96
#define LED_OFF_FULL 1
#if DEBUG
...
static u32 should_increase;
static u32 status_change;//plug in out delay
static u32 delay_times;
static u32 first_get;//we must read and update
static u32 have_finish;//poweroff charging 2 poweron complete flag lost
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		break;
		/*end to add by malei at 2013-01-09*/
	default:
		return -EINVAL;
	}
	return 0;
...
static int msm_batt_get_batt_chg_status(void)
{
    ...
    /*begain to  add by yanggang for fix bug 433154 2013-4.24*/
       struct rpc_reply_batt_chg_v2 *v2p;
	...
	if (rc < 0) {
		...
		be32_to_cpu_self(v1p->is_charging_complete);
             be32_to_cpu_self(v2p->is_charging);
        	pr_debug( "ygg **** %s %d is_charging_complete %d  is_charging %d\n", 
             /*end to  add by yanggang for fix bug 433154 2013-4.24*/
	__func__,__LINE__,v1p->is_charging_complete,v2p->is_charging);
	} else {
...
static void led_on(int on){
    static int pre ;
     pr_info( "ygg *******%s %d on %d\n", __func__,__LINE__,on);
	gpio_set_value(BATT_LED_GPIO,on);
	should_increase = on;
    if(pre != on){
        status_change = 1;
         pr_info( "ygg *******insert event %s %d \n", __func__,__LINE__);
    }
    pre = on;
      delay_times =0;
	 //printk(KERN_DEBUG "ygg *******%s %d on %d\n", __func__,__LINE__,on);
}
/*begain to  add by yanggang for beetle little at 2013-1.25*/
void msm_batt_update_led_status(void)
{
	DBG_LIMIT( "ygg ****enter %s %d  \n",__func__,__LINE__);

    if(100 == msm_batt_info.batt_capacity){
		 printk(KERN_DEBUG "ygg *** full off led %s %d  \n",__func__,__LINE__);
		 wake_unlock(&(msm_batt_info.eoc_wake_lock));
		led_on(0);
	}else if (msm_batt_info.charger_status == CHARGER_STATUS_GOOD ||
		msm_batt_info.charger_status == CHARGER_STATUS_WEAK) 
	{
		if (msm_batt_info.current_chg_source) {
            wake_lock(&(msm_batt_info.eoc_wake_lock));
			led_on(1);
		}
	} else {
	wake_unlock(&(msm_batt_info.eoc_wake_lock));
...
/*end to  add by yanggang for beetle little at 2013-1.25*/

/*begain to  add by yanggang for fix bug 411779 2013-3.12*/

static void msm_batt_update_once(int battery_voltage)
{

	u32     cap_now,update,is_charging_complete;

	update = 0;
    cap_now = 50;
    DBG_LIMIT( "ygg **** %s %d                        battery_voltage         %d\n", __func__,__LINE__,battery_voltage);
    ...
    if(should_increase)
    {
        ...
        {
            DBG_LIMIT( "chg ignore %d %d\n", battery_voltage,msm_batt_info.battery_voltage );
        }
        // cause we judge full anther way
        if(battery_voltage > 4198)
        {
            update =1;
            DBG_LIMIT( "check full \n");
            if (msm_batt_get_batt_chg_status())
		return;
            is_charging_complete = rep_batt_chg.v1.is_charging_complete;  
     
            if(is_charging_complete){
              printk(KERN_DEBUG "ygg ****enter %s %d is_charging_complete %d \n",__func__,__LINE__,is_charging_complete);
             msm_batt_info.is_charging_complete = is_charging_complete;
    ...
    if (update) 
    {
        ...
        cap_now = msm_batt_info.calculate_capacity(battery_voltage);
        if (should_increase  && cap_now > msm_batt_info.batt_capacity) 
        {
        	update =1;
        }else if( !should_increase  && cap_now < msm_batt_info.batt_capacity) 
        {
        	update =1;
        }else {
          DBG_LIMIT( "ignore pre %d %d  \n", 
          msm_batt_info.batt_capacity, cap_now);
        update =0;
        }
    }
	...
	if (update) 
	{
	    msm_batt_info.battery_voltage  	= battery_voltage;
   		...
   		msm_batt_info.batt_capacity = cap_now;
		printk(KERN_DEBUG "ygg  %s record  in %d capacity %d \n", __func__,__LINE__,msm_batt_info.batt_capacity);	
...
{
	static u32 unnecessary_event_count,pre;
       static unsigned long chg_complete_time;
	u32	charger_status;
	u32	charger_type;
	u32	battery_status;
	u32	battery_level;
	...
	battery_temp = rep_batt_chg.v1.battery_temp;
      is_charging_complete = rep_batt_chg.v1.is_charging_complete;
      if (rep_batt_chg.v2.is_charging){
        DBG_LIMIT( "ygg ****enter %s %d  \n",__func__,__LINE__);
              msm_batt_info.boot_no = 0;
      }
     
      if(is_charging_complete){
          printk(KERN_DEBUG "ygg ****enter %s %d is_charging_complete %d \n",__func__,__LINE__,is_charging_complete);
        chg_complete_time = jiffies;
         msm_batt_info.is_charging_complete = is_charging_complete;
        //led 
        msm_batt_info.battery_voltage = battery_voltage;
      ...
      //finish 5m   and poweroff 2 power on complete flag lost
    if( msm_batt_info.is_charging_complete)
    {
        if(jiffies-chg_complete_time > 5*60*HZ){
            printk(KERN_DEBUG "ygg ****enter %s %d is_charging_complete %d \n",__func__,__LINE__,is_charging_complete);
            msm_batt_info.is_charging_complete = is_charging_complete;
        }else
        {
            printk(KERN_DEBUG "ygg ****enter %s %d stay 5m is_charging_complete %d \n",__func__,__LINE__,is_charging_complete);
        }
    }else{
        DBG_LIMIT("ygg ****enter %s %d is_charging_complete %d \n",__func__,__LINE__,is_charging_complete);
        msm_batt_info.is_charging_complete = is_charging_complete;
    }
	...
	if( battery_temp >= 40 || battery_temp <= 0){
		msm_batt_info.batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
		msm_batt_info.charger_status = CHARGER_STATUS_BAD;
		pr_info( "temp is not ok  %s %d on %d\n", __func__,__LINE__,battery_temp );
	}

    if(pre != msm_batt_info.current_chg_source){
        //status_change = 1;
         pr_info( "ygg *******insert event %s %d on %d\n", __func__,__LINE__,pre);
    }
    pre = msm_batt_info.current_chg_source;
			...
			
	{
	  ...
	  wake_unlock(&(msm_batt_info.eoc_wake_lock));
	  led_on(0);
              //here remodify it cause led_on switched off 
              should_increase=1;
              have_finish=1;
              delay_times=0;
	}
	...
	if (msm_batt_info.battery_voltage != battery_voltage) {
		pr_info( "resume update  %s %d  %d\n", __func__,__LINE__,battery_voltage );
		msm_batt_update_once(battery_voltage);
		if (!supp)
			supp = msm_batt_info.current_ps;
	}
	msm_batt_update_led_status();
...
void msm_batt_early_suspend(struct early_suspend *h)
{
	...
	#if LED_OFF_FULL
	printk(KERN_DEBUG "ygg *******%s %d\n", __func__,__LINE__);
	#else
	cancel_delayed_work(&msm_batt_info.battery_work);
	#endif
	//cancel_delayed_work(&msm_batt_info.battery_work);
...
void msm_batt_late_resume(struct early_suspend *h)
{
	...
	//printk(KERN_DEBUG "ygg *******%s %d\n", __func__,__LINE__);
	rc = queue_delayed_work(msm_batt_info.battery_queue,
				   &msm_batt_info.battery_work,
				   0);
...
static int msm_batt_cleanup(void)
{
	...
	if(msm_batt_info.battery_queue)
		destroy_workqueue(msm_batt_info.battery_queue);
        /*end to  add by yanggang for fix bug 411779 2013-3.12*/
#ifdef CONFIG_HAS_EARLYSUSPEND
...
/*begain to  add by yanggang for fix bug 433154 2013-4.24*/
static u32 msm_batt_vol_convert(u32 current_voltage_raw)
{
        const static u32 chg_to_dis_table[][2] = {
        {4190,  35},
        {4140,  80},
        {4120,  85},
        {3960,  90},
    ...
    u32 i ;
    for (i = 1; i < sizeof(chg_to_dis_table) / sizeof(
    chg_to_dis_table[0]); i++) {
        if (current_voltage_raw >= chg_to_dis_table[i][0]) {
            break;
        }
    }
 
    return current_voltage_raw - chg_to_dis_table[i][1];
}

/*begain to  add by yanggang for fix bug 433154 2013-4.24*/
static u32 msm_batt_capacity(u32 current_voltage_raw)
{
	const static u32 c_voltage_to_capacity_table[][2] = {
	   {4199,  99},
	   {4175,  95},
	   {4152,  90},
	   {4130,  85},
    ...
    u32 i, base, top, percent_diff,current_voltage;

    current_voltage= current_voltage_raw;
    if(should_increase){  
          
		DBG_LIMIT( "ygg ****use chg table  %s %d  \n",__func__,__LINE__);

		if ((msm_batt_info.is_charging_complete) && 
		    (current_voltage_raw >= 4000))
		{
		    pr_info( " **** %s %d  \n",__func__,__LINE__);
		     msm_batt_vol_convert(4000);
		    return 100;
		}
		else if (current_voltage >= 4199)
		{
		    return 99;
		}


		for (i = 1; i < sizeof(c_voltage_to_capacity_table) / sizeof(
		c_voltage_to_capacity_table[0]); i++) {
		    if (current_voltage >= c_voltage_to_capacity_table[i][0]) {
		        break;
		    }
		}

		base = c_voltage_to_capacity_table[i][0];
		top = c_voltage_to_capacity_table[i - 1][0];
		percent_diff =  c_voltage_to_capacity_table[i - 1][1] -
		    c_voltage_to_capacity_table[i][1];
		return c_voltage_to_capacity_table[i][1] +
		    (current_voltage - base) * percent_diff / (top - base);

   }else
   {
	DBG_LIMIT( "ygg ****use dis table  %s %d  \n",__func__,__LINE__);
	if (current_voltage >= 4160)
	{
	    return 100;
	}else if (current_voltage >= 4158)
	{
	    return 99;
	}
	for (i = 1; i < sizeof(voltage_to_capacity_table) / sizeof(
	voltage_to_capacity_table[0]); i++) {
...
static void msm_batt_worker(struct work_struct *work)
{
	...
	u32     battery_voltage,cap_now,update,is_charging_complete;
    ...
    if(first_get){
        ...
        cap_now = msm_batt_info.calculate_capacity(battery_voltage);
        pr_info( " %d  voltage = %u mV [capacity = %d%%]\n",__LINE__,
         battery_voltage, cap_now);
         first_get =0;
         update=1;
         goto update;
    }
    ...
    if(delay_times < 3) {
        ...
        printk(KERN_DEBUG "ygg **** %s %d        insert ignore      \n", __func__,__LINE__);
       //I can see you but can do nothing,cancle delay
      //goto done;
        }
    else {status_change = 0;
    delay_times =0;
    }
    }
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
...
#endif
	gpio_tlmm_config(GPIO_CFG(BATT_LED_GPIO, 0,
	...
	INIT_DELAYED_WORK(&msm_batt_info.battery_work, msm_batt_worker);
	

	queue_delayed_work(msm_batt_info.battery_queue,
			   &msm_batt_info.battery_work,
			   (3 * HZ));
	wake_lock_init(&(msm_batt_info.eoc_wake_lock), WAKE_LOCK_SUSPEND, "msm_battery");
