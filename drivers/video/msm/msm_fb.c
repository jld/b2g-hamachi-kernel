...
/* zhouzhijun and the code for back and white  during press the pow key quickly.
 * zhouzhjun@tcl.com  	2013.03.15
 */
...
//       printk("enter msm_fb_set_bl_brightness\n");
	/* This maps android backlight level 0 to 255 into
	   driver backlight level 0 to bl_max with rounding */
	bl_lvl = (2 * value * mfd->panel_info.bl_max + MAX_BACKLIGHT_BRIGHTNESS)
		/(2 * MAX_BACKLIGHT_BRIGHTNESS);
//	printk("XXX dp***%s value:%x, bl_lvl:%x.\n", __func__, value, bl_lvl);
	if (!bl_lvl && value)
		bl_lvl = 1;
/* zhouzhijun and the code for back and white  during press the pow key quickly.
 * zhouzhjun@tcl.com  	2013.03.15
 */
/*to fix PR 429009 also pay attention to 411431  yanggang@tcl.com*/
		down(&mfd->sem);
...
//modified by dupeng@tcl.com for bug 438313 (dispaly a flick screen after alcatel one touch screen when power off charge)
//static int unset_bl_level, bl_updated;
static int unset_bl_level = 0x80;
static int bl_updated = 0;
//modified by dupeng end
...
{
	/* sunzhangyang and the code for backlight can not be lighted.
           zhangyang.sun@tcl.com  	2013.05.07
       */

        struct msm_fb_panel_data *pdata;
	__u32 temp = bkl_lvl;
        #if DEBUG_SUN 
        pr_info("enter msm_fb_set_backlight------.\n");
        #endif
	if (!mfd->panel_power_on || !bl_updated){
        ...
        #if DEBUG_SUN 
        pr_info("msm_fb_set_backlight**** bkl_lvl=%d\n",bkl_lvl);
        #endif
	pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
			      ...
			      struct fb_info *info)
{
	...
	struct msm_fb_panel_data *pdata;
        #if DEBUG_SUN 
	pr_info("msm_fb_pan_display entry\n");
        #endif
	/*
	...
	up(&msm_fb_pan_sem);
//printk("$$$ dp***%s unset_bl_level:%x,bl_updated:%x.\n", __func__, unset_bl_level, bl_updated);
        #if DEBUG_SUN 
	pr_info("msm_fb_pan_display unset_bl_level=%d, bl_updated=%d\n", unset_bl_level, bl_updated);
        #endif
	if (unset_bl_level && !bl_updated) {
        ...
        #if DEBUG_SUN 
	pr_info("msm_fb_pan_display exit\n");
        #endif
