...
static int mipi_sleep_on = 0; //add by dupeng for lcd sleep mode 2013-04-20
...
//add by dupeng for LCD power cum 2013-04-20
void mipi_dsi1_ulps_mode(int on)
{
	int reg_val, reg_set_v = 0;
	
	reg_set_v = MIPI_INP(MIPI_DSI_BASE + 0xa8);
	reg_set_v &= 0x10000000; 
	if(on)
	{
		//printk("%s:ULPS_REQUEST start:%x, status:%x.\n", __func__, MIPI_INP(MIPI_DSI_BASE + 0xa8) , MIPI_INP(MIPI_DSI_BASE + 0xa4));
		reg_set_v |= (DLN1_ULPS_REQUEST | 1<<0 | 1<<2 | 1<<4);//DSI1_CLKLN_EN | DSI1_DLN1_EN;
		MIPI_OUTP(MIPI_DSI_BASE + 0xa8, reg_set_v);
		wmb();
		mdelay(5);
		reg_val = MIPI_INP(MIPI_DSI_BASE + 0xa4);
		//printk("%s:ULPS_REQUEST end:%x,status:%x.\n", __func__, MIPI_INP(MIPI_DSI_BASE + 0xa8), reg_val);
		if((reg_val & 0x1700) != 0)
			printk("%s:on lane1 error.\n", __func__);
		else
			printk("%s:enter successfully.\n", __func__);
	}
	else
	{
		//printk("%s:ULPS_REQUEST start:%x,status:%x.\n", __func__, MIPI_INP(MIPI_DSI_BASE + 0xa8), MIPI_INP(MIPI_DSI_BASE + 0xa4));
		reg_val = MIPI_INP(MIPI_DSI_BASE + 0xa4);
		reg_set_v |= (DLN1_ULPS_EXIT | 1<<10 | 1<<8 | 1<<12);
		MIPI_OUTP(MIPI_DSI_BASE + 0xa8, reg_set_v);
		wmb();
		mdelay(5);
		reg_val = MIPI_INP(MIPI_DSI_BASE + 0xa4);
		...
		if((reg_val & 0x1700) != 0)
		{
			reg_set_v = MIPI_INP(MIPI_DSI_BASE + 0x00a8); 
			reg_set_v &= 0x10000000; // in order to keep bit28 
			MIPI_OUTP(MIPI_DSI_BASE + 0x00a8, reg_set_v); 
			//printk("%s exit ulps mode successfuly.\n", __func__);
		}
		else
			printk("%s exit ulps mode error.\n", __func__);
	}
}
//dupeng add end
static int mipi_dsi_off(struct platform_device *pdev)
{
	...
	//printk("enter mipi_dsi_off.\n");

	mfd = platform_get_drvdata(pdev);
	pinfo = &mfd->panel_info;
//add by dupeng for lcd exit sleep 2013-04-20
	if(!mipi_sleep_on)
	{
...
//dupeng add end
	if (mipi_dsi_pdata && mipi_dsi_pdata->dsi_power_save)
			mipi_dsi_pdata->dsi_power_save(1); //.dsi_power_save		= mipi_dsi_panel_power,
