...
struct wake_lock powerkey_lock;
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	case KEY_VOLUMEDOWN:
                #if DEBUG_SUN
		pr_info("enter report_hs_key-------------\n");
                #endif
               if((key==KEY_END) && (!wake_lock_active(&powerkey_lock)))
                {
                   wake_lock_timeout(&powerkey_lock,2 * HZ);
                } 
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	int rc = 0;
	struct input_dev *ipdev;
        wake_lock_init(&powerkey_lock, WAKE_LOCK_SUSPEND, "POWER_KEY");
