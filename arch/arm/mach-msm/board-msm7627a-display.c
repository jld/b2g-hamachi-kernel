...
#include <mach/gpio.h>
#include <mach/msm_iomap.h> /*add malei*/

#define DEBUG_SUN 0 /*add sun zhangyang*/

#ifdef CONFIG_FB_MSM_TRIPLE_BUFFER
#define MSM_FB_SIZE		0x4BF000
//#define MSM7x25A_MSM_FB_SIZE    0x1C2000
#define MSM7x25A_MSM_FB_SIZE    0x4BF000
#define MSM8x25_MSM_FB_SIZE	0x5FA000
#define MSM7627A_QRD3_FB_SIZE	0x1C2000
#else
...
//#ifdef CONFIG_LCDC_TOSHIBA
		...
		if (!strncmp(name, "mipi_cmd_renesas_fwvga", 22))
			ret = 0;
#endif
#ifdef CONFIG_FB_MSM_MIPI_DSI_ILI9486
        if (!strncmp(name, "mipi_cmd_ili9486_hvga", 21))
			ret = 0;
#endif
...
#define GPIO_LCD_BL_PWM 27      //add by zhaiqingbo 2013.1.21
...
/*add by malei  : if the gaia do not change the backlight control interface ,it should add the back light to here*/
#define MAX_BACKLIGHT_BRIGHTNESS 255
static struct clk *gp_clk; /*GPIO 27 as function 2 pwm*/
extern int clk_enable(struct clk *clk);
extern void clk_disable(struct clk *clk);
void enable_gp_clock(int on)
{
	static int count = 0;
	if (on) {
		if (count <= 0)
...
{
    static int first_switch_mode=0;
	unsigned int r_read58=0;
	unsigned int r_read5c=0;
        #if DEBUG_SUN
	pr_info("backlight_set entry level=%d\n", level);
        #endif
	gpio_set_value(17, 1);
	if (level > MAX_BACKLIGHT_BRIGHTNESS)
	{
             level = MAX_BACKLIGHT_BRIGHTNESS;
	}

    if(!first_switch_mode)
    {
        first_switch_mode =1;
        gpio_tlmm_config(GPIO_CFG(GPIO_LCD_BL_PWM, 1,
			GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA),
			GPIO_CFG_ENABLE);
    }
    ...
    if(level > 0)
    {
        ...
        unsigned short duty = (n_val / (MAX_BACKLIGHT_BRIGHTNESS + 1)) * level;    //max: ~18.33mA
	...
	r_read58=__raw_readl(MSM_CLK_CTL_BASE + 0x58);
	r_read5c=__raw_readl(MSM_CLK_CTL_BASE + 0x5c);
        #if DEBUG_SUN
	pr_info("r_read58=0x%x,r_read5c=0x%x\n",r_read58,r_read5c);
        #endif
	return 0;
};
...
static struct msm_panel_common_pdata mipi_pdata = {
	.pmic_backlight = backlight_set,   //matain by malei
	.gpio = -1,
};
static struct platform_device mipi_dsi_panel_device = {
#ifdef CONFIG_FB_MSM_MIPI_DSI_RENESAS
	.name = "mipi_renesas",
#elif defined(CONFIG_FB_MSM_MIPI_DSI_ILI9486)         /* will be added to configuration file later by zhaiqingbo  */
	.name = "mipi_ili9486",                /* product.  */
#else
	.name = "mipi_unknown",
#endif
	.id = 0,
...
void __init msm_msm7627a_allocate_memory_regions(void)
{
	...
	else if (machine_is_msm7x27a_ffa())
	{
		...
		pr_info("7627a_qrd3 allocating %lu bytes for fb\n", fb_size);
	}
	else
		fb_size = MSM_FB_SIZE;
...
#define GPIO_LCDC_BRDG_RESET_N	85 //zhaiqingbo modify 2013.1.21
...
static struct regulator_bulk_data regs_dsi[] = {
	{ .supply = "gp2",   .min_uV = 2850000, .max_uV = 2850000 },//zhaiqingbo modify 2013.1.21
...
static struct mipi_dsi_platform_data mipi_dsi_pdata = {
	.vsync_gpio		= MDP_303_VSYNC_GPIO,//zhaiqingbo modify for LCD TE, 2013.3.1
...
void msm7x27a_set_display_params(char *prim_panel)
{
	if (strnlen(prim_panel, PANEL_NAME_MAX_LEN)) {
		...
		printk("msm_fb_pdata.prim_panel_name %s\n",
