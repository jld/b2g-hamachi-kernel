...
#define MSM7x25A_MSM_PMEM_MDP_SIZE       0x800000 //0x1500000

#define MSM_PMEM_ADSP_SIZE      0x1200000
#define MSM7x25A_MSM_PMEM_ADSP_SIZE      0xD00000  //0xB91000
...
static struct msm_psy_batt_pdata msm_psy_batt_data = {
	...
	/*begain to  add by yanggang for fix bug 407140 2013-3.08*/
	.voltage_max_design 	= 4175,
	/*end to  add by yanggang for fix bug 407140 2013-3.08*/
...
static u32 step_f(u32 current_voltage,u32 low_voltage,u32 width)
{
...
static u32 msm_calculate_batt_capacity(u32 current_voltage)
{
 	...
 	else if( current_voltage <= 3600)
	{
	    low_voltage=l1;
		width = w1;
	}else if(current_voltage <= 3700)
	{
	    low_voltage=l2;
		width = w2;
	}else if(current_voltage <= 3840)
	{
	    low_voltage=l3;
		width = w3;
	}else if(current_voltage < 4175)
	{
	    low_voltage=l4;
		width = w4;
	}else{
	printk(KERN_ERR "ygg  fix me  %d \n", current_voltage);
	}

 	return step_f(current_voltage,low_voltage,width);
}
...
{
#ifdef CONFIG_ANDROID_PMEM
    /*add 7x27a_ffa for Beetle-lite-ff by zhaiqingbo 2013.2.17 */
	if (machine_is_msm7625a_surf() || machine_is_msm7625a_ffa() || machine_is_msm7x27a_ffa()) {
...
/* zhaiqingbo delete it for lcd continue display, 2013.3.7, 
    CLK mode will change GPIO27 low, turn off LCD backlight */
#if 0
/*add by malei for back light */
#define GPIO_LCD_BL_PWM 27
...
/*end add by malei for backlight */
#endif 
...
/* zhaiqingbo delete it for lcd continue display,2013.3.7 */
#if 0
/* add by malei for backlight init*/
    backlight_init();
/*end add by malei*/
#endif
