...
static long snd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SND_SET_DEVICE:
		...
		/* add begin by wangdongliang for GSM receiver 2013.2.2 */
		if (0==dmsg.args.device) {
			MM_INFO("en snd device is %d\n", dmsg.args.device);
			pmic_spkr_bypass_en(LEFT_SPKR, 1);
			pmic_spkr_bypass_en(RIGHT_SPKR, 1);
		}
		/* add end */
